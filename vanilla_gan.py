# For training the DCGAN
#
# At least some of the code was taken from assignments from CSC421 
#
# Usage:
# ======
...
def save_model(G, fixed_noise, iteration, opts):
    #Save model checkpoint
    save_obj(G, "Trained Model_" + str(iteration))
    save_obj(opts, "Model Opts_" + str(iteration))
...
def training_loop(train_dataloader, opts):
    ...
    for epoch in range(opts.num_epochs):
        ...
        for batch in train_dataloader:
            ...
            D_real_loss = lossfn(real_output, utils.to_var(torch.ones(real_output.shape[0])))
            ...
            D_fake_loss = lossfn(fake_output, utils.to_var(torch.zeros(fake_output.shape[0])))
            ...
            fake_output = D(fake_images)
            G_loss = lossfn(fake_output, utils.to_var(torch.ones(fake_output.shape[0])))
            ...
            if iteration % opts.sample_every == 0:
                ...
                save_model(G, fixed_noise, iteration, opts)
...
def main(opts):
    ...
    train_dataloader, _ = get_emoji_loader(opts)
...
def create_parser():
    ...
    # Data sources
    parser.add_argument('--emoji', type=str, default='Apple', choices=['dog'],
